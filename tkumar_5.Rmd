...
#Silhouette method shows that 8 numbers of clusters would be optimum.  

k8 <- kmeans(ImputedDF, centers = 8, nstart = 25)
fviz_cluster(k8, geom = "point",  data = ImputedDF) + ggtitle("k = 8")


# slicing the dendogram on the longest path, 8 is the optimal level of clusters.
...
#From the Dendrogram it seems thatclustering at level of 5 would have been optimum. 
pltree(hc_ward, cex = 0.6, hang = -1, main = "Dendrogram of agnes")
rect.hclust(hc_ward, k = 5, border = 1:4)

#Thus, Hierarchical clustering would be better approach than kmeans to find the suitable no. of clusters.
...
ggplot(nbclust_plot) +
  ...
  theme_minimal()
#Based on all 30 indices, the best number of clusters is 5 clusters.
...
rect.hclust(hc_train, k = 5)

CWcut <- cutree(hc_train, k = 5)
...
CWclust4 <- CWtotal[CWtotal$CWcut==4,]
 ...
 CWclust5 <- CWtotal[CWtotal$CWcut==5,]
 colMeans(CWclust5)
 
CWmeans1 <- rbind(colMeans(CWclust1),colMeans(CWclust2),colMeans(CWclust3),colMeans(CWclust4) ,colMeans(CWclust5))
head(CWmeans1)
